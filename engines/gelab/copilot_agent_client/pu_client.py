...
def evaluate_task_on_device(agent_server, device_info, task, rollout_config, extra_info = {}, reflush_app=True, auto_reply = False, reset_environment=True, report_path=None, image_dir=None):
    """
    ...
    for step_idx in range(max_steps):
        ...
        # smart_remove(image_path) # Delayed removal for visualization
        ...
        # Visualization: Draw points and add to report
        if report_path and image_dir and os.path.exists(image_path):
            try:
                points_to_draw = []
                act_type = action.get('action_type', '') or action.get('action', '')
                if "point" in action:
                    points_to_draw.append(action['point'])
                if "point1" in action:
                    points_to_draw.append(action['point1'])
                if "point2" in action:
                    points_to_draw.append(action['point2'])
                
                action_img_name = f"{session_id}_step_{step_idx+1}_action.png"
                action_img_path = os.path.join(image_dir, action_img_name)
                
                draw_points(image_path, action_img_path, points_to_draw)
                
                append_images_as_table(
                    [(f"Step {step_idx+1}: {act_type}", action_img_path)],
                    report_path
                )
            except Exception as e:
                print(f"Visualization error: {e}")
        
        # Now remove the temp screenshot
